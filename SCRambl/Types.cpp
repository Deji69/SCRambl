...
using namespace SCRambl::Types;

const DataSourceSet Attributes<DataSourceID, DataSourceSet>::s_AttributeSet;
const DataAttributeSet Attributes<DataAttributeID, DataAttributeSet>::s_AttributeSet;
...
ValueSet SCRambl::Types::GetValueTypeByName(std::string name) {
...
SCRambl::Types::Basic* Type::ToBasic() { return IsBasicType() ? static_cast<Basic*>(this) : nullptr; }
SCRambl::Types::Extended* Type::ToExtended() { return IsExtendedType() ? static_cast<Extended*>(this) : nullptr; }
SCRambl::Types::Variable* Type::ToVariable() { return IsVariableType() ? static_cast<Variable*>(this) : nullptr; }
const SCRambl::Types::Basic* Type::ToBasic() const { return IsBasicType() ? static_cast<const Basic*>(this) : nullptr; }
const SCRambl::Types::Extended* Type::ToExtended() const { return IsExtendedType() ? static_cast<const Extended*>(this) : nullptr; }
const SCRambl::Types::Variable* Type::ToVariable() const { return IsVariableType() ? static_cast<const Variable*>(this) : nullptr; }
...
/* Types */
Translation::Ref SCRambl::Types::Types::AddTranslation(TypeRef<Type> type, ValueSet valuetype, size_t size) {
...
void SCRambl::Types::Types::AddValueAttributes(XMLConfig* type) {
	auto value = type->AddClass("Number", [this](const XMLNode vec, void*& obj){
		...
		auto value = type->AddValue<NumberValue>(m_Types, type->GetID(), numtype, vec["Size"]->AsNumber<size_t>(0));
		AddValue(ValueSet::Number, value);
		obj = value;
	});
	auto text = type->AddClass("Text", [this](const XMLNode vec, void*& obj){
		auto type = static_cast<Type*>(obj);
		auto value = type->AddValue<TextValue>(m_Types, type->GetID(), vec["Size"]->AsNumber<uint32_t>(), *vec["Mode"]);
	...
	auto command = type->AddClass("Command", [this](const XMLNode vec, void*& obj) {
		...
		if (datatype == DataType::INVALID)
			BREAK();
		auto value = type->AddValue<CommandValue>(m_Types, type->GetID(), vec["Size"]->AsNumber<uint32_t>(), vec["Value"]->AsString(), datatype);
		AddValue(ValueSet::Command, value);
		obj = value;
	});
	auto operation = type->AddClass("Operation", [this](const XMLNode vec, void*& obj) {
		auto type = static_cast<Type*>(obj);
		auto datatype = DataType::GetByName(vec["Type"]->AsString());
		if (datatype == DataType::INVALID)
			BREAK();
		auto value = type->AddValue<Operators::OperationValue>(m_Types, type->GetID(), vec["Size"]->AsNumber<uint32_t>(), vec["Value"]->AsString(), datatype);
		AddValue(ValueSet::Operation, value);
		obj = value;
	});
	auto label = type->AddClass("Label", [this](const XMLNode vec, void*& obj) {
		auto type = static_cast<Type*>(obj);
		auto scope_attr = *vec["Scope"];
		auto value = type->AddValue<LabelValue>(m_Types, type->GetID(), vec["Size"]->AsNumber<uint32_t>(), scope_attr);
...
void SCRambl::Types::Types::Init(Build& build) {
